...
//        ArrayList<int[]> path = solver.dfs();
        ...
        BFS bfs = new BFS();
        bfs.setMaze(scanner);
        humanSTART = bfs.getPlayerSTART();
        end = bfs.getEnd();
        ArrayList<int[]> path = bfs.bfs();
        ...
        startScene = new Scene(startGroup, bfs.getMaze().get(0).size() * TILE_SIZE, bfs.getMaze().size() * TILE_SIZE);
...
//        Label startLabel = new Label(title);
        ...
        mazeGroup = new Group();
        for (int i = 0; i < bfs.getMaze().size(); i++) {
            ArrayList<Integer> row = bfs.getMaze().get(i);
...
//        });
        ...
        mainScene = new Scene(mazeGroup, bfs.getMaze().get(0).size() * TILE_SIZE, bfs.getMaze().size() * TILE_SIZE);
        ...
        mainScene.setOnKeyPressed(event -> {
            ...
            switch (event.getCode()) {
                case UP:
                    player.moveUp(bfs.getMaze(), player.getPosition());
                    ...
                    break;
                case DOWN:
                    player.moveDown(bfs.getMaze(), player.getPosition());
                    playerRect.setY(player.getPosition()[0] * TILE_SIZE);
                    ...
                    break;
                case LEFT:
                    player.moveLeft(bfs.getMaze(), player.getPosition());
                    ...
                    break;
                case RIGHT:
                    player.moveRight(bfs.getMaze(), player.getPosition());
                    playerRect.setX(player.getPosition()[1] * TILE_SIZE);
                    human = new double[]{playerRect.getX(), playerRect.getY()};
                    robot = new double[]{entityRect.getX(), entityRect.getY()};
                    winAlert(human,robot);
